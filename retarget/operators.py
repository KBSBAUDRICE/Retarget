            ...
            if self.align:
                for src_name, trg_name in bone_names_map.items():
                    ...
                    try:
                        ...
                        sr_bone.matrix = tj_bone.matrix.copy()
...
class ConstrainToArmature(Operator):
    ...
    def invoke(self, context, event):            
        ...
        if to_bind.data.retarget_retarget.has_settings():
            self.src_preset = '--Current--'
        if context.active_object.data.retarget_retarget.has_settings():
        ...
        for ob in armatures:
            ...
            for src_name in bone_names_map.keys():
                ...
                if src_name == src_skeleton.root:
                    if self.constrain_root == "None":
                        continue
                    if self.constrain_root == "Bone" and not self.root_motion_bone and not self.root_motion_bone_sr:
                        continue
                try:
            ...
            if self.constrain_root == 'Object' and self.root_motion_bone and self.root_motion_bone_sr:
            ...
            if self._constrained_root and self.root_motion_bone and self.root_motion_bone_sr:
...
class AddRootMotion(Operator):
    ...
    def execute(self, context):
        ...
        for ob in armatures:
            ...
            if not self.root_motion_bone in ob.pose.bones:
