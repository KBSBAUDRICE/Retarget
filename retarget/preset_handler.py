...
PRESETS_SUBDIR = os.path.join("armature", "humanoid")
...
def set_preset_skel(preset, context, validate=True):
    ...
    if validate:
        validate_preset(context.active_object.data)
...
def get_preset_skel(preset, settings=None):
    ...
    code.body.pop(0)  # remove line 'skeleton = context.object.data.retarget_retarget' from preset
